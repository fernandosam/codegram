...
function main(container, outline, toolbar, sidebar, status) {
  dataTypes = loadDataTypes('./data/datatypes.txt'); // Checks if the browser is supported
  ...
  if (!mxClient.isBrowserSupported()) {
    ...
    var mxVertexToolHandler = function mxVertexToolHandler(state) {
      mxVertexHandler.apply(this, arguments);
    };

    // Global Variables
    var createRelationshipAction = false;
    var createTableAction = false; // Specifies shadow opacity, color and offset
    ...
    mxConstants.EDGE_SELECTION_DASHED = false; // Remove expanded and collapsed images

    mxGraph.prototype.expandedImage = null;
    mxGraph.prototype.collapsedImage = null; // Table icon dimensions and position

    mxSwimlane.prototype.imageSize = 15;
    mxSwimlane.prototype.imageDx = 16;
    mxSwimlane.prototype.imageDy = 7; // Changes swimlane icon bounds
    ...
    mxVertexToolHandler.prototype.init = function () {
      mxVertexHandler.prototype.init.apply(this, arguments); // In this example we force the use of DIVs for images in IE. This
      ...
      this.domNode.style.whiteSpace = 'nowrap'; // Workaround for event redirection via image tag in quirks and IE8
      ...
      var img = createImage('images/relation.png'); //img.setAttribute('title', 'Connect');
    ...
    checkImage.src = "images/check.png"; // Workaround for Internet Explorer ignoring certain CSS directives
    ...
    var model = graph.model; // Disables some global features

    graph.setConnectable(true);
    graph.setCellsDisconnectable(false);
    graph.setCellsCloneable(false);
    graph.swimlaneNesting = false;
    graph.dropEnabled = true; // Do not allow removing labels from parents

    graph.graphHandler.removeCellsFromParent = false; // Does not allow dangling edges

    graph.setAllowDanglingEdges(false); // Forces use of default edge in mxConnectionHandler

    graph.connectionHandler.factoryMethod = null; // Extends Canvas
    ...
    graph.createHandler = function (state) {
      if (state != null && graph.isHtmlLabel(state.cell)) {
        showPanelColumn(graph, state.cell);
      }

      if (state != null && this.model.isVertex(state.cell) && state.cell.style == 'table') {
    ...
    editor.configure(config); // Configures the automatic layout for the table columns
    ...
    editor.createSwimlaneLayout = function () {
      ...
      layout.fill = true;
      layout.resizeParent = true; // Overrides the function to always return true
    ...
    graph.getLabel = function (cell) {
      if (this.isHtmlLabel(cell)) {
        ...
        var fkValue = cell.value.foreignKey ? 'FK' : ''; //console.log(cell.value.dataType);
        ...
        table = table + "<div style='color:#B15DFF;'>" + fkValue + "</div></div>"; //mxUtils.htmlEntities(cell.value.name, false) + '&#9;' + mxUtils.htmlEntities(cell.value.dataType.mysql, false);
    ...
    graph.addListener(mxEvent.CLICK, function (sender, evt) {
      if (createTableAction) {
        ...
        var pt = mxUtils.convertPoint(graph.container, mxEvent.getClientX(me), mxEvent.getClientY(me));
    ...
    configureStylesheet(graph); // Adds table icon
    //var table = Table.prototype.newCell();
    ...
    graph.addEdge = function (edge, parent, source, target, index) {
      ...
      try {
        var col1 = this.model.cloneCell(column); //col1.value.name = primaryKey.value.name;
    ...
    openProject(editor, 'project'); // Creates the outline (navigator, overview) for moving
    // around the graph in the top, right corner of the window.

    var outln = new mxOutline(graph, outline); // Fades-out the splash screen after the UI has been loaded.
...
function showModalWindow(title, content, width, height) {
  ...
  wnd.setClosable(true); // Fades the background out after after the window has been closed
...
function addSidebarIcon(graph, sidebar, prototype, image) {
  ...
  sidebar.appendChild(img); // Creates the image which is used as the drag icon (preview)

  var dragImage = img.cloneNode(true);
  var ds = mxUtils.makeDraggable(img, graph, funct, dragImage); // Adds highlight of target tables for columns
...
function configureStylesheet(graph) {
  ...
  style[mxConstants.STYLE_IMAGE_HEIGHT] = '48'; //style[mxConstants.STYLE_GRADIENTCOLOR] = '#FFFFFF';
  ...
  style[mxConstants.STYLE_STROKEWIDTH] = '5'; //style[mxConstants.STYLE_FONTSTYLE] = 1;
  //style[mxConstants.STYLE_IMAGE] = 'images/icons48/table.png';
  // Looks better without opacity if shadow is enabled
  //style[mxConstants.STYLE_OPACITY] = '80';
  //style[mxConstants.STYLE_SHADOW] = 1;

  graph.getStylesheet().putCellStyle('table', style); //style = graph.stylesheet.getDefaultEdgeStyle();
  ...
  style[mxConstants.STYLE_VERTICAL_ALIGN] = mxConstants.ALIGN_MIDDLE;
  style[mxConstants.STYLE_EDGE] = mxEdgeStyle.EntityRelation; //style[mxConstants.STYLE_STARTARROW] = mxConstants.ARROW_OVAL;
...
function showPanelColumn(graph, cell) {
  ...
  var typeParamField = formColumn.addText('Custom Size', cell.value.typeParam); // Form Constraints
  ...
  var defaultField = formConstraint.addText('Default', cell.value.defaultValue || ''); // Form Options

  var formOptions = new mxForm('properties');
  var autoIncrementField = formOptions.addCheckbox('Auto Increment', cell.value.autoIncrement); // Form to key

  var formKey = new mxForm('properties');
  var primaryKeyField = formKey.addCheckbox('Primary Key', cell.value.primaryKey);
  var foreignKeyField = formKey.addCheckbox('Foreign Key', cell.value.foreignKey); // Function to be executed when change values
  ...
  destroyAllPanelElements(); // Panel Elements
...
function showPanelTable(graph, cell) {
  ...
  var nameField = formTable.addText('Name', cell.value.name); // Function to be executed when change values
...
function foreignKeysConfig(graph) {
  ...
  var childCount = graph.model.getChildCount(parent); // ForeignKeys
...
function createSql(graph) {
  ...
  var parent = graph.getDefaultParent();
  var childCount = graph.model.getChildCount(parent); // Models
      ...
      if (columnCount > 0) {
        for (var j = 0; j < columnCount; j++) {
          ...
          if (column.foreignKey) {
            ...
            var sqlFk = 'ALTER TABLE ' + table + ' ADD FOREIGN KEY (FK_' + table + '_' + columnName + '_' + ref.table + ') REFERENCES ' + ref.table + '(' + ref.column + ')';
