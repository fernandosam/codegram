...
function loadDataTypes(filename) {
  ...
  var idx = 0;
  var dataTypes = []; // Parses all lines
...
function saveProject(editor, name) {
  if (name != null) {
    ...
    try {
      ...
      console.log(e); //editor.setStatus(mxUtils.htmlEntities(mxResources.get('errorSavingFile')));
    }
  }
}
...
function createPanelElement(title, content, okFunction) {
  var c = document.getElementById('panel-right'); // Summary

  var summary = document.createElement('summary');
  summary.innerText = title; // Details
